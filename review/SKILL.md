...
> Review 自身不执行 `dotnet test`。测试执行由 `tdd-execute` skill（流水线 §8.8）独立负责。
...
| TDD 覆盖 | TDD 设计文档是否存在且完整 | `design/06-tdd/{slug}-tdd-design.md` 存在 |
...
| TDD闭环一致性 | ✅/❌ | design/06-tdd/ → test/ → api/ |
| API功能完备性 | ✅/❌ | workflow + frontend + raw-input → api/ |
| 标准与架构合规 | ✅/❌ | path检查 + domain-registry + schema |
